...
> Hinweis: Diese Version nutzt themoviedb.org statt thetvdb.com als
> Datenquelle, da thetvdb.com aktuell nicht erreichbar war. TMDB ist
> ebenfalls kostenlos nutzbar und kommt sogar ohne separaten Login-Schritt
> aus – ein API-Key reicht.
...
4. **Karte als Ressource registrieren (einmaliger, manueller Schritt):**
   Anders als frühere Versionen dieser Anleitung bindet sich die Karte
   **nicht mehr automatisch** ein – das erwies sich als unzuverlässig
   (sporadischer "Konfigurationsfehler", siehe Technische Hinweise unten).
   Stattdessen wie bei jeder anderen Custom Card üblich:
...
- **Eigenes Icon in "Geräte & Dienste"**: Seit Home Assistant 2026.3 können
  ...
  angezeigt.
- **Umbenennung**: Das Projekt hieß zwischenzeitlich `tmdb_series_tracker`.
  Falls du eine ältere Version installiert hattest, ist das hier technisch
  eine neue Integration (neue Domain `ha_seriesdb`) – die alte Integration
  in "Einstellungen → Geräte & Dienste" entfernen, den alten
  `custom_components/tmdb_series_tracker`-Ordner löschen und `ha_seriesdb`
  frisch einrichten. Die Watchlist muss dabei leider neu angelegt werden, da
  sie pro Domain gespeichert wird.
